...
function addData() {
    ...
    let review1 = db.review.create({
        title: "Making the cake",
        comment: "Amazing cake recipes \
        after reading this book I feel like Gordon Ramsey"
    })

    let feedback1 = db.feedback.create({
        name: "Martin Chorley",
        feedback: "This Website is amazing \
        I love it."
    })


    // wait for all the objects to save and then instantiate relationships.
    Promise.all([book1, book2, book3, author1, author2, author3, user1, loan1, review1, feedback1]).then(function(
        results
    ) {
        b1 = results[0];
        b2 = results[1];
